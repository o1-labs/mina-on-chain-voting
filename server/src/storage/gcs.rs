...
impl GcsProvider {
    pub async fn new(project_id: &str, service_account_key_path: Option<&str>) -> Result<Self> {
        // Try to create authenticated client first, but fall back to anonymous HTTP access for public buckets
        let client = if let Some(_path) = service_account_key_path {
            ...
            match ClientConfig::default().with_auth().await {
                ...
                Err(err) => {
                    tracing::warn!("Failed to initialize GCS with service account authentication, using anonymous HTTP access for public buckets: {}", err);
    ...
    async fn list_objects(&self, bucket: &str, prefix: Option<&str>) -> Result<Vec<String>> {
        match &self.client {
            GcsClient::Authenticated(client) => {
                ...
                tracing::info!("GCS authenticated client found {} objects in bucket '{}': {:?}", 
                    objects.len(), bucket, objects.iter().take(5).collect::<Vec<_>>());
                ...
                loop {
                    ...
                    if response.status().is_client_error() {
                        if response.status() == 401 || response.status() == 403 {
                            return Err(anyhow!("GCS bucket '{}' requires authentication. Please set GCS_PROJECT_ID and optionally GCS_SERVICE_ACCOUNT_KEY_PATH environment variables.", bucket));
                        }
                    ...
                    if page_token.is_none() || page_count >= 10 {
                        if page_count >= 10 {
                            tracing::warn!("Stopped fetching GCS objects after {} pages ({} objects) to avoid excessive API calls", page_count, all_objects.len());
                        }
                        break;
                    }
                }
                
                tracing::info!("GCS anonymous client found {} objects across {} pages in bucket '{}': {:?}", 
                    all_objects.len(), page_count, bucket, all_objects.iter().take(5).collect::<Vec<_>>());
    ...
    async fn get_object(&self, bucket: &str, key: &str) -> Result<Bytes> {
        match &self.client {
            ...
            GcsClient::Anonymous(http_client) => {
                ...
                if response.status().is_client_error() {
                    if response.status() == 401 || response.status() == 403 {
                        return Err(anyhow!("GCS object '{}' in bucket '{}' requires authentication. Please set GCS_PROJECT_ID and optionally GCS_SERVICE_ACCOUNT_KEY_PATH environment variables.", key, bucket));
                    }
                    return Err(anyhow!("Failed to access GCS object '{}' in bucket '{}': HTTP {}", key, bucket, response.status()));
                }
