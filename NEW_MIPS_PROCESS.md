...
- [Calculation of Mina’s On-Chain Voting Results](#calculation-of-minas-on-chain-voting-results)
  - [Overview](#overview)
  - [Results Calculation Instructions](#results-calculation-instructions)
...
3. Add a new entry to the list of proposals, following the format of the existing entries
   ...
   - the `url` should be a link to the MIP's proposal on the [Mina Protocol MIPs repository](https://github.com/MinaProtocol)
   - the network should be `mainnet` or `devnet`
   - the `ledger_hash` must be the ledger hash of the epoch after the voting epoch (e.g. for voting in epoch 53, use the ledger hash of epoch 55)
...
1. Obtain the next staking ledger of the next voting epoch
...
Since we are calculating the results for MIP3 and MIP4 voting (epoch 53), we need the next staking
ledger of the next epoch, i.e. the staking ledger of epoch 55.
...
- [Zk Validator's mina-graphql-rs repo](https://github.com/zkvalidator/mina-graphql-rs/blob/main/data/epochs/)
- [Granola’s mina-ledger repo](https://github.com/Granola-Team/mina-ledger/tree/main/mainnet)
...
- Mina Explorer has [BigQuery](https://docs.minaexplorer.com/minaexplorer/bigquery-public-dataset), [GraphQL](https://docs.minaexplorer.com/minaexplorer/graphql-getting-started), and [REST](https://docs.minaexplorer.com/rest-api/ref) APIs
...
3. Filter the votes
   - _memo.lower()_ exactly equal to _mip3_ or _no mip3_
...
### Calculate weighted voting results
  a. Sum all aggregated voter stake to get the total voting stake
  b .For each delegate, start with their total stake, and subtract the balances of accounts that delegate
  to them with an overriding vote
  c. Divide yes/no vote stakes by the total voting stake
...
Conclusion
MIP voting is a substantial addition to Mina’s ever-evolving suite of on-chain governance
functionality. This is a huge milestone which advances the community and decentralization of the
protocol. Granola is honored to be a part of the technical implementation of such a major
development in the Mina ecosystem.
